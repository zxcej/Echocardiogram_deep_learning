...
class VideoDataset(Dataset):
    ...
    def __init__(self, dataset , split='train', clip_len=16, preprocess=False):
        ...
        if 'cardiac' in self.dataset :
            self.resize_height = 256 #437 , 256 ,128
            self.resize_width = 256  #630 , 256, 128
            self.crop_size = 255 #255 ,128
        else:
        ...
        #mask = mask.resize((self.resize_width,self.resize_height))
        ...
        if dataset == "ucf101":
            ...
            if not os.path.exists('cardiac_labels.txt'):
                with open('cardiac_labels.txt', 'w') as f:
                    for id, label in enumerate(sorted(self.label2index)):
                        f.writelines(str(id+1) + ' ' + label + '\n')
        exc_dir = os.path.join(self.output_dir, 'ex_vivo_analysis_values2.0 - Copy.xlsx')
    ...
    def __getitem__(self, index):
        ...
        buffer = self.load_frames(self.fnames[index]) #shape: t,c,h,w
        buffer = self.normalize(buffer)

        labels = np.array(self.label_array[index]).astype(np.longlong)
        
        #############################################################TESTING REGRESSION ON ENERGY LOSS VALUES
        #el_labels = self.get_tabular_data(index)
    ...
    def get_tabular_data(self, indx):
        ...
        info = (self.tab_data[cur_sample_row,1:]).astype(np.float)
    ...
    def preprocess(self):
        print("Started Pre-Processing step.......")
        if not os.path.exists(self.output_dir):
        ...
        if self.dataset == 'cardiac_mini_vids' :
            ...
            while (count < frame_count and retaining):
                ...
                if count % EXTRACT_FREQUENCY == 0:
                    ...
                    cv2.imwrite(filename=os.path.join(save_dir, video_filename,'0000{}{}.jpg'.format(str(i),str(video_filename))), img=frame)
                    i += 1
                count += 1
            print(video_filename," Has ",count," frames")
    ...
    def process_mini_video(self, video_filename, save_dir, capture):
        ...
        for vid_ind in range(num_mini_vid):
            if not os.path.exists(os.path.join(save_dir, video_filename+"_minivid_"+str(vid_ind))):
                ...
                os.mkdir(os.path.join(save_dir, video_filename+"_minividFlip_"+str(vid_ind)))
        ...
        while (total_count < frame_count and retaining):
            ...
            cv2.imwrite(filename=os.path.join(save_dir, video_filename+"_minividFlip_"+str(mini_vid_indx), '0000{}.jpg'.format(str(i))), img=frame_flip)
    ...
    ##HELPER Function to understand pixel distribution:
    ...
    def load_frames(self, file_dir):
        ...
        for i, frame_name in enumerate(frames[:frame_count]):
