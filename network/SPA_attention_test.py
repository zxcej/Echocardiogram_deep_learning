...
from helper_functions import forward_train,forward_test,plot_results,plot_sample_data, impl_weighted_sampler
...
if __name__ == '__main__':
    ...
    test_dataset = VideoDataset(dataset='cardiac_mini_vids', split='test',  clip_len=16)

    train_dataloader , test_dataloader = impl_weighted_sampler(train_dataset,test_dataset, 16)
    sample = next(iter(train_dataloader))[0][0].permute(1,0,2,3).to(device)
    print(sample.shape)
    plt.figure()

    plt.imshow(sample[0].cpu().detach().permute(1,2,0).numpy())
    plt.figure()
    out,att_map = net(sample,return_spa_map=True)
    tst_img = out[0]
    plt.imshow(tst_img.cpu().detach().permute(1,2,0).numpy())

    print("SPA net output size",out.size())
    print(att_map[0].shape)
    plt.figure()
    plt.imshow(att_map[0][0].cpu().detach().numpy())
    ###########################################################################
    spa = SPABlock(in_channels=3, k=8, adaptive=True).to(device)
    out = spa(img)
    print("SPA Block output size",out.shape)
